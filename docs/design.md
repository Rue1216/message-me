...
標籤與全文搜尋是兩條各自獨立的檢索路徑：標籤自內文抽離之後不再是內文的一部分，而 `sp_post_search` 只比對 `posts.content`，因此搜尋標籤文字不會命中只把它掛成標籤的那篇發文——要沿著主題瀏覽請走標籤頁。這是刻意的分工而非搜尋漏掉了：標籤是結構化的分類欄位，全文搜尋找的是使用者真正寫下的句子。
...
這個決定也反過來約束了功能的實作方式：標籤必須以「業務層正規化 → 單一 SP 寫入」的形式完成，
...
4. 驗證**延伸功能**：標籤的指定與瀏覽、按讚的冪等性、他人個人檔案頁的合併時間軸、
